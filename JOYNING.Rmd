...
ui <- navbarPage(
    fluid = TRUE,
    "NotOkCupid",
     navbarMenu("GET TO KNOW YOUR CUPID",
              tabPanel("Histogram",
    ...
    tabPanel("Fun Facts About Cupid",
    ...
    tabPanel("Component 1",
             ...
             sidebarPanel(
                 ...
                 selectInput("yv", "Your Choise",
                            choice = c("height","income","offspring_1")),
             ),
             mainPanel(
                 plotOutput("distPlot"),
             )),
...
server <- function(input, output, session) {
    ...
    output$distPlot <- renderPlot({
        # generate bins based on input$bins from ui.R
      if(input$yv == "income" | input$yv = "height" ) {
       if(input$yv == "income"){
            y <- MyCupid[,10]
        }else if(input$yv == "height"){
            y <- MyCupid[,9]}
       bins <- seq(min(y), max(y), length.out = input$bins + 1)
        #draw the histogram with the specified number of bins
        hist(y, breaks = bins, col = 'lightpink2', border = 'white',xlab=input$yv,main = paste("Histogram of",input$yv))
      }
      else {
